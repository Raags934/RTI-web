...
export class PrioritizationOne implements OnInit {
      ...
      if (event.type === 'applyFilter') {
        ...
        const basePayload: PrioritizationPayload = {
          ideas: rankedIdeas.map((c) => {
            ...
            const currentRank = c.ranking_brand;
    ...
    this.ideas.forEach(idea => {
      if (!this.originalRankings.has(idea.idea_id)) {
        this.originalRankings.set(
          idea.idea_id,
          idea.ranking_brand != null ? String(idea.ranking_brand) : null
        );
      }
    });
  ...
  private reorderPayloadWithLockedRanks(payload: PrioritizationPayload): PrioritizationPayload {
    const ideas = (payload.ideas as ProductRankingChange[]).map((item, index) => {
      const parsedRank = Number(item.ranking_brand);
      return {
...
const N = maxUserRank
    ...
    const reorderedIdeas: ProductRankingChange[] = slots
      .flatMap((item, index) =>
        item === null ? [] : [{ idea_id: item.idea_id, ranking_brand: String(index + 1), lock: item.locked }]
      );
  ...
  /** Check if all displayed ideas are ranked (only for Product Prioritization Pending filter) */
  get allIdeasRanked(): boolean {
    // Only validate when in "Product Prioritization Pending" filter (status_id 10)
    if (this.currentFilterStatusId !== 10) {
      return true; // Allow submission in other filters
    }

    // Check if all displayed ideas have rankings
    const displayedIds = new Set(this.displayedIdeas.map((i) => i.idea_id));
    const allRanked = this.displayedIdeas.every((idea) => {
      const rankingChange = this.rankingChanges.find((c) => c.idea_id === idea.idea_id);
      return rankingChange?.ranking_brand != null && rankingChange.ranking_brand !== '';
    });
  ...
  submitRanking() {
    // Check if all ideas are ranked (only for Product Prioritization Pending filter)
    if (this.currentFilterStatusId === 10 && !this.allIdeasRanked) {
      // Show error popup
